      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative">
          ...
          <button onClick={() => scroll(isAr ? 'right' : 'left')}
            className="absolute start-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-sm flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 -ms-4">
          ...
          <div ref={scrollRef}
            ...
            {items.map((brand: any) => (
              <Link key={brand.id} href={`/${locale}/brands/${brand.slug}`}
                className="flex-shrink-0 group flex flex-col items-center gap-2 w-24 py-3 px-2 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                <div className="w-14 h-14 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm flex items-center justify-center overflow-hidden p-1.5">
                  {brand.logo_url
          ...
          <button onClick={() => scroll(isAr ? 'left' : 'right')}
            className="absolute end-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full shadow-sm flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-700 -me-4">
            <svg className="w-4 h-4 text-gray-600 dark:text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
