      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="flex justify-end mb-6">
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {/* Country filter — grouped right beside the sort filters, matching their style. */}
            {countries.length > 0 && (
              <details className="relative">
                <summary className={`list-none [&::-webkit-details-marker]:hidden cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeCountry ? 'bg-white/10 text-white' : 'bg-white/[0.03] text-neutral-400 hover:bg-white/10'}`}>
                  <span>🌍</span>
                  <span>{activeCountry || (isAr ? 'كل الدول' : 'All countries')}</span>
                  <span className="text-[9px] opacity-60">▾</span>
                </summary>
                <div className="absolute z-30 mt-2 end-0 min-w-[170px] max-h-72 overflow-auto rounded-xl border border-white/10 bg-[#11151b] p-1.5 shadow-2xl">
