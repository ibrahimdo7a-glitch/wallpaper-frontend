...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  return {
    title: locale === 'ar' ? 'الأكثر إعجابًا - منصة الخلفيات' : 'Most Liked - Wallpaper Platform',
  };
}

export default async function MostLikedPage({ params: { locale }, searchParams }: Props) {
  ...
  let wallpapers = null;
  try {
    const res = await wallpaperApi.list({ sort: 'most_liked', page, per_page: 30 });
    wallpapers = res.data;
  } catch {}
