...
export async function generateMetadata({ params: { locale } }: { params: { locale: Locale } }): Promise<Metadata> {
  return {
    title: locale === 'ar' ? 'الشروط والأحكام | QEV' : 'Terms & Conditions | QEV',
  };
}
...
export default async function TermsPage({ params: { locale } }: { params: { locale: Locale } }) {
  ...
  const custom = (isAr ? site?.terms_ar : site?.terms_en)?.trim();
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {custom ? (
        // Admin-managed content (Site Settings → الشروط والأحكام)
        <div className="article-body text-gray-300" dangerouslySetInnerHTML={{ __html: custom }} />
      ) : (
        // Default fallback when no custom terms have been written yet
