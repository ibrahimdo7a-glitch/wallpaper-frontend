...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const [brand, { section }] = await Promise.all([
    ...
    fetchSectionContent(params.slug, params.section, 1, 1),
  ]);
  if (!brand || !section) return { title: 'Not Found' };
  ...
  return { title: `${sectionName} — ${brandName} | QEV` };
}
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default async function BrandSectionPage({ params, searchParams }: Props) {
  const isAr = params.locale === 'ar';
    ...
    <main className="min-h-screen bg-black text-white" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          {brand.logo_url && (
            <Image src={brand.logo_url} alt={brandName} width={32} height={32} className="object-contain" />
          )}
